...
class warehouse:
    ...
    def get_path(self):
        ...
        path.append([[self.start.x, self.start.y],'u'])
        path.reverse()
        print path
        return path
    ...
    def movement(self, route_id):
        ...
        for kx in self.route_id:
            while 1:
                ...
                self.flag = self.striper(self.ax)
                #self.flag = self.ax
                if str(self.flag) == str(kx[0]):
                    if self.objsel==1:
                        r1=marker_ref[kx[0]]
                    if self.objsel==2:
                        r2=marker_ref[kx[0]]
...
if __name__ == "__main__":
    ...
    while 1:
        ...
        for ind, obj in enumerate(object_list):
            ...
            routes = obj.solve()
            if routes == None:
                print 'item no.{} cannot be picked due to limitations'.format(ind+1)
                continue
